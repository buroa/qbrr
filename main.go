...
func runAnnounce(ctx context.Context, client client.Client, opts *config.Options) error {
	if len(opts.TorrentFilterOptions.Hashes) != 0 {
		if ok, err := client.WaitForTrackerUpdate(ctx, opts.TorrentFilterOptions.Hashes[0]); err != nil {
			return fmt.Errorf("failed to wait for tracker update: %w", err)
		} else if ok {
			slog.Debug("Torrent is ok, skipping reannouncement", "hash", opts.TorrentFilterOptions.Hashes[0])
			return nil
		}
	}
	...
	for _, torrent := range torrents {
		...
		go func(t qbittorrent.Torrent) {
			defer wg.Done()
			if utils.ShouldReannounce(t, opts.MaxAge) {
				if err := client.ReannounceTorrentWithRetry(ctx, t.Hash, &opts.ReannounceOptions); err != nil {
					...
					tracker, _ := utils.GetTLDPlusOne(t.Tracker)
					slog.Info("Reannounced torrent successfully", "name", t.Name, "hash", t.Hash, "tracker", tracker)
				}
			}
		}(torrent)
	}
