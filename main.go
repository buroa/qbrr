...
func process(ctx context.Context, client client.Client, torrent qbittorrent.Torrent, opts *config.Options) {
	...
	if utils.IsTrackerStatusUpdating(torrent.Trackers) {
		if ok, err := client.WaitForTrackerUpdate(ctx, torrent.Hash); err != nil {
			slog.Error("Failed to wait for tracker update", "name", torrent.Name, "hash", torrent.Hash, "error", err)
		} else if ok {
			slog.Debug("Torrent tracker ok'd, skipping reannouncement", "name", torrent.Name, "hash", torrent.Hash)
			return
		}
	}

	if err := client.ReannounceTorrentWithRetry(ctx, torrent.Hash, &opts.ReannounceOptions); err != nil {
		slog.Error("Failed to reannounce torrent", "name", torrent.Name, "hash", torrent.Hash, "error", err)
	} else {
		tracker, _ := utils.GetTLDPlusOne(torrent.Tracker)
		slog.Info("Reannounced torrent successfully", "name", torrent.Name, "hash", torrent.Hash, "tracker", tracker)
	}
}
