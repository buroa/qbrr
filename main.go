...
func process(ctx context.Context, client client.Client, torrent qbittorrent.Torrent, opts *config.Options) {
	...
	if utils.IsTrackerStatusUpdating(torrent.Trackers) {
		if ok, err := client.WaitForTrackerUpdate(ctx, torrent.Hash); err != nil {
			slog.Warn("Failed to wait for tracker update, reannouncing torrent anyway", "hash", torrent.Hash, "tracker", tracker, "error", err)
		} else if ok {
			slog.Debug("Tracker update successful, skipping reannouncement", "hash", torrent.Hash, "tracker", tracker)
			return
		}
	}

	if err := client.ReannounceTorrentWithRetry(ctx, torrent.Hash, &opts.ReannounceOptions); err != nil {
		slog.Error("Failed to reannounce torrent", "hash", torrent.Hash, "tracker", tracker, "error", err)
	} else {
		slog.Info("Reannounced torrent successfully", "hash", torrent.Hash, "tracker", tracker)
	}
}
