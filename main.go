...
func process(ctx context.Context, client client.Client, torrent qbittorrent.Torrent, opts *config.Options) {
	...
	if utils.IsTrackerStatusUpdating(torrent.Trackers) {
		slog.Info("Waiting for tracker update", "hash", torrent.Hash, "tracker", tracker)
		...
		if ok, err := client.WaitForTrackerUpdate(ctx, torrent.Hash); err != nil {
			...
			if ok {
				slog.Info("Tracker update OK - skipping", "hash", torrent.Hash, "tracker", tracker)
				return
			}
			slog.Info("Tracker update not OK - proceeding with reannounce", "hash", torrent.Hash, "tracker", tracker)
		}
	}
